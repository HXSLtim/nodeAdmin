...
| TD-2 | 解决 `react-intl@10.1.0` 与 React 18 的 peer 依赖冲突，恢复 `npm install` 可重建 lockfile 的能力（当前只有 `npm ci` 能干净装）。**2026-04-08 D-021：走第 (d) 条路径，降级 react-intl 到最高的 react@^18 peer 版本（T-P5-FE-01 执行中）**                                                               | 中（执行中）      | D-021              |
